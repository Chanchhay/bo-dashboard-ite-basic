        ...
        return (
            <Panel>
                <div className="flex min-h-48 flex-col items-center justify-center gap-3 px-6 text-center">
                    <span className="grid size-11 place-items-center rounded-full bg-[#f2f3f1] text-[#5c6660]">
                        <Lock className="size-5" aria-hidden="true" />
                    </span>
                    <p className="text-[15px] text-[#16181c]">
                        You don&apos;t have access to the audit log
                    </p>
                    <p className="max-w-md text-[14px] text-[#8a8f89]">
                        Viewing audits needs the{" "}
                        <code className="rounded bg-[#f2f3f1] px-1.5 py-0.5 text-[13px]">
                            admin-audit:read
                        </code>{" "}
                        ...
                        <table className="w-full min-w-[760px] border-collapse text-left">
                            ...
                            <tbody>
                                {logs.map((log) => (
                                    <tr
                                        key={log.id}
                                        className="border-b border-[#f2f3f1] align-top last:border-0"
                                    >
                                        <td className="py-4 pr-4 text-[14px] whitespace-nowrap text-[#5c6660]">
                                            {formatTimestamp(log.createdAt)}
                                        </td>
                                        <td className="py-4 pr-4 text-[14px] text-[#16181c]">
                                            {log.actorUsername ||
                                                log.actorId ||
                                                "—"}
                                        </td>
                                        ...
                                        <td className="py-4 text-[13px] text-[#8a8f89]">
                                            {log.previousState ||
                                            log.newState ? (
                                                <span>
                                                    {log.previousState || "—"}
                                                    {" → "}
                                                    <span className="text-[#16181c]">
                                                        {log.newState || "—"}
                                                    </span>
                                                </span>
                                            ) : (
