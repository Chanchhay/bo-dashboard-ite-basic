...
import { useOpenRegisterMutation } from "@/features/pin/cash-register-api";
import { setRegisterSession } from "@/features/sessionSlice";


export function CashRegister({ onClose }: { onClose?: () => void }) {
  const router = useRouter();
  const dispatch = useAppDispatch();
  const { cashierId, businessOwnerId} = useAppSelector((s) => s.session);

  const [amount, setAmount] = useState("0.00");
  const [notes, setNotes] = useState("");
  const [error, setError] = useState("");
  const [openRegister, { isLoading }] = useOpenRegisterMutation();
  ...
  const handleOpenRegister = async () => {
    ...
    try {
      const { registerSessionId } = await openRegister({
        businessOwnerId,
        cashierId,
        startingCash: parseFloat(amount),
